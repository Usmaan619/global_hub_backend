...
// done final and working properly but daily record count is not added under admin user
// exports.getSuperAdminAndAdminDetailsCount_old = async (req, res) => {
      ...
      if (admin_id) {
        const [monthlyUserRecordStatsRaw] = await connection.query(
          `SELECT
           ...
           LEFT JOIN records r ON r.user_id = u.id
           WHERE u.admin_id = ? AND YEAR(r.created_at) = ?
           GROUP BY u.username, MONTH(r.created_at)
